...
var RadarChart = {
  draw: function(id, data, options) {
    ...
    function render(data) {
      ...
      if (config.facet) {
        data.forEach(function(d, i) {
          ...
          vis.svg.selectAll(".polygon-vertices").attr("fill", config.colors(i));
          vis.svg.selectAll(".legend-tiles").attr("fill", config.colors(i));
        });
      } else {
    ...
    function buildVisComponents() {
      ...
      vis.allAxis = data[0].axes.map(function(i, j) {
        return i.axis;
      });
      ...
      vis.verticesTooltip = d3
        .select("body")
        .append("div")
        .classed("verticesTooltip", true)
        .attr("opacity", 0)
        .style({
          position: "absolute",
          color: "black",
      ...
      for (var level = 0; level < config.levels; level++) {
        ...
        vis.levels
          ...
          .attr("x1", function(d, i) {
            return (
              levelFactor * (1 - Math.sin((i * config.radians) / vis.totalAxes))
            );
          })
          .attr("y1", function(d, i) {
            return (
              levelFactor * (1 - Math.cos((i * config.radians) / vis.totalAxes))
            );
          })
          .attr("x2", function(d, i) {
            return (
              levelFactor *
              (1 - Math.sin(((i + 1) * config.radians) / vis.totalAxes))
            );
          })
          .attr("y2", function(d, i) {
            return (
              levelFactor *
              (1 - Math.cos(((i + 1) * config.radians) / vis.totalAxes))
            );
          })
          .attr(
            "transform",
            "translate(" +
              (config.w / 2 - levelFactor) +
              ", " +
              (config.h / 2 - levelFactor) +
              ")"
          )
          .attr("stroke", "gray")
    ...
    function buildAxes() {
      vis.axes
        ...
        .attr("x2", function(d, i) {
          return (
            (config.w / 2) *
            (1 - Math.sin((i * config.radians) / vis.totalAxes))
          );
        })
        .attr("y2", function(d, i) {
          return (
            (config.h / 2) *
            (1 - Math.cos((i * config.radians) / vis.totalAxes))
          );
        })
        .attr("stroke", "grey")
    ...
    function buildAxesLabels() {
      vis.axes
        ...
        .attr("x", function(d, i) {
          return (
            (config.w / 2) *
            (1 - 1.3 * Math.sin((i * config.radians) / vis.totalAxes))
          );
        })
        .attr("y", function(d, i) {
          return (
            (config.h / 2) *
            (1 - 1.1 * Math.cos((i * config.radians) / vis.totalAxes))
          );
        })
        .attr("font-family", "sans-serif")
        ...
        group.axes.forEach(function(d, i) {
          d.coordinates = {
            ...
            x:
              (config.w / 2) *
              (1 -
                (parseFloat(Math.max(d.value, 0)) / config.maxValue) *
    ...
    function buildPolygons(data) {
      vis.vertices
        ...
        .attr("points", function(group) {
          // build verticesString for each group
          var verticesString = "";
        ...
        .on(over, function(d) {
          vis.svg
            .selectAll(".polygon-areas") // fade all other polygons out
            .transition(250)
